...
def send_scheduled_message():
    try:
        ...
        user_ids_file = os.path.join(os.path.dirname(__file__), 'user_ids.txt')
        with open(user_ids_file, "r") as file:
            user_ids = set(file.read().splitlines())
...
def handle_message(event):
    ...
    if event.message.text == "a":
        ...
        msg3 = TextSendMessage(text='周報表心率測試')
        line_bot_api.reply_message(event.reply_token, msg3)

if __name__ == "__main__":
    schedule_jobs()
    t = threading.Thread(target=schedule.run_pending)
    t.start()
    app.run()
